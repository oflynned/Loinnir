...
public class PartnerConversationFrag extends Fragment {
    ...
    public void onStart() {
        ...
        JSONObject partnerMessagesPayload = new JSONObject();
        try {
            partnerMessagesPayload.put("my_id", LocalStorage.getID(getActivity()));
            partnerMessagesPayload.put("partner_id", partner.getId());
        } catch (JSONException e) {
            e.printStackTrace();
        }

        RestClient.post(getActivity(), Endpoints.GET_PARTNER_MESSAGES, partnerMessagesPayload, new BaseJsonHttpResponseHandler<JSONArray>() {
            @Override
